...
# requirements — so there is no custom aggregation. The dev env installs the
# modules editable (referencing $REPO_ROOT), so source edits are live; the
# test env is the production wheels plus the dev group, for sandboxed runs.
#
# Usage:
...
let
  ...
  #
  # OCB's setup.cfg also sets `[install] optimize=1`, which makes setuptools
  # byte-compile the wheel *indirectly*: it spawns a fresh interpreter on a
  # script that does `from distutils.util import byte_compile`. That resolves
  # to the stdlib on 3.11 and to nothing on 3.12+ (distutils is gone, and the
  # setuptools shim only activates through a .pth that a PYTHONPATH-based
  # build env never processes). install_lib skips byte-compilation altogether
  # when bytecode writing is off, so turn it off: the wheel ships no .pyc,
  # which is how every other package in the set ships anyway.
