...
After `odoo-add-module`, run `direnv reload` so the Nix engine re-derives `addons_path` and
rebuilds the Python env.

## Python environment

The project's `pyproject.toml` + `uv.lock` are the single Python manifest, built reproducibly
by **uv2nix** (no pip, no `.venv`). The root is a virtual package (`[tool.uv] package =
false`); Odoo itself runs from the `odoo/` submodule source, so only its third-party
dependencies are installed. `[project].dependencies` is the union of:

- **Odoo core** — translated from `odoo/requirements.txt` (already valid PEP 508 specifiers);
- **OCA modules' `external_dependencies.python`** — aggregated **scoped to the install
  closure** (`modules.txt` + transitive `depends`), so unrelated modules in a cloned repo
  don't drag in conflicting deps.

Two managed blocks keep this maintenance-free (regenerated by `odoo-add-module`/`odoo-update`):
the OCA dep block in `[project].dependencies`, and a `[tool.uv.extra-build-dependencies]`
block that grants `setuptools` to sdist-only legacy packages so uv2nix can build them.
`lib/overrides.nix` supplies the few native-build overrides (psycopg2 → `libpq` +
`pg_config`; python-ldap → openldap, patched off the legacy `ldap_r` lib). For a genuine
OCA-vs-Odoo version conflict, use `[tool.uv] override-dependencies`.
...
lib/
  ...
  oca-lib.sh                 # OCA resolver + module picker (shell)
  oca_pydeps.py              # scoped OCA Python-dep aggregation
  uv_build_deps.py           # sdist build-dep sync
  init.nix / odoo-init.sh    # the scaffolder
  odoo-presets.json          # series → python / branch
data/                        # vendored OCA catalog + generator scripts
